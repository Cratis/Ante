...
/// </summary>
/// <remarks>
/// <c language="csharp">app.MapFallbackToFile("index.html")</c> answers every request that matches no other endpoint with
/// a 200 response containing the SPA shell - including a mistyped or unknown path under <c language="csharp">/api</c>,
/// <c language="csharp">/openapi</c>, <c language="csharp">/_invite</c> or <c language="csharp">/healthz</c>. A caller probing for API surface, or a client
...
/// prefix (for example <c language="csharp">/api/register-organization</c>) is more specific than this catch-all and still
...
/// tooling; anywhere else, a request under <c language="csharp">/openapi</c> falls through to <see cref="ApiRouteGuard"/>'s
