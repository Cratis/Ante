...
public class and_a_create_tenant_invitation_arrives : Specification
{
    static readonly EventSourceId _invitationId = (EventSourceId)Guid.NewGuid().ToString();
    static readonly UserInvitedToCreateTenant _invited = new("jane@example.com", ["Owner"]);
    ...
    void should_record_the_invitation_locally() =>
        _scenario.ShouldHaveProduced<CreateTenantInvitationReceived>(e => e.Email == _invited.Email);
}
#endif
