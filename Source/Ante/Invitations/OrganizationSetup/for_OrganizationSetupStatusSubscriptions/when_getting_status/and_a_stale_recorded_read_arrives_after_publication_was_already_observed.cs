...
public class and_a_stale_recorded_read_arrives_after_publication_was_already_observed : Specification
{
    ...
    {
        _subscriptions = new();
        _subscriptions.GetStatus(_invitationId, "Acme", isRecorded: true, isFullyPublished: true);
    }
    ...
    [Fact] void should_still_be_accepted() => Assert.Equal(OrganizationSetupAcceptanceStatus.Accepted, _result.Status);
}
#endif
